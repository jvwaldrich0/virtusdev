...
void send_barcode(int fd, const char *barcode) {
    ...
    printf("%s[BARCODE SCAN]%s Sending: %s%s%s\n", COLOR_CYAN, COLOR_RESET, COLOR_GREEN, barcode, COLOR_RESET);
    printf("%s[BARCODE SCAN]%s Length: %d characters at %d baud\n", COLOR_CYAN, COLOR_RESET, len, BAUDRATE);
    ...
    printf("%s[BARCODE SCAN]%s Complete! (took %ld μs, %.2f ms)\n", COLOR_CYAN, COLOR_RESET, duration_us, duration_us / 1000.0);
    printf("%s[BARCODE SCAN]%s Effective rate: %.0f chars/sec\n\n", COLOR_CYAN, COLOR_RESET,
...
int find_virtual_keyboard() {
    ...
    if (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (strncmp(line, "event", 5) == 0) {
            ...
            printf("%s[DEVICE]%s Found virtual keyboard at: %s%s%s\n", 
                   COLOR_BLUE, COLOR_RESET, COLOR_GREEN, device_path, COLOR_RESET);
...
void print_header(void) {
    printf("%s╔═══════════════════════════════════════════════════════╗%s\n", COLOR_CYAN, COLOR_RESET);
    printf("%s║         BARCODE READER EMULATOR (115200 baud)        ║%s\n", COLOR_CYAN, COLOR_RESET);
    printf("%s╚═══════════════════════════════════════════════════════╝%s\n", COLOR_CYAN, COLOR_RESET);
    printf("  %sDevice%s     : %s\n", COLOR_YELLOW, COLOR_RESET, DEVICE_NAME);
    printf("  %sBaudrate%s   : %d bps\n", COLOR_YELLOW, COLOR_RESET, BAUDRATE);
    printf("  %sChar Time%s  : %d μs\n", COLOR_YELLOW, COLOR_RESET, CHAR_TIME_US);
    printf("  %sUser%s       : jvwaldrich0\n", COLOR_YELLOW, COLOR_RESET);
...
int main(int argc, char *argv[]) {
    ...
    if (device_arg) {
        ...
        if (fd < 0) {
            perror("%s[ERROR]%s Cannot open device");
            return 1;
        }
        printf("%s[DEVICE]%s Using: %s%s%s\n\n", COLOR_BLUE, COLOR_RESET, COLOR_GREEN, device_arg, COLOR_RESET);
    } else {
        fd = find_virtual_keyboard();
        if (fd < 0) {
            printf("%s[ERROR]%s Cannot find virtual keyboard device\n", COLOR_RED, COLOR_RESET);
            printf("%s[ERROR]%s Make sure virtual_keyboard is running!\n\n", COLOR_RED, COLOR_RESET);
            print_usage(argv[0]);
            return 1;
        }
        printf("\n");
    ...
    if (is_interactive) {
        printf("%s[MODE]%s Interactive barcode scanner mode\n", COLOR_GREEN, COLOR_RESET);
        printf("%s[INFO]%s Enter barcode data and press Enter (Ctrl+D to exit)\n", COLOR_BLUE, COLOR_RESET);
        printf("%s[INFO]%s Each line will be sent as a complete barcode scan\n\n", COLOR_BLUE, COLOR_RESET);
    } else {
        printf("%s[MODE]%s Pipe/redirect mode - reading from stdin\n\n", COLOR_GREEN, COLOR_RESET);
    }
    
    while (1) {
        if (is_interactive) {
            printf("%sSCAN>%s ", COLOR_CYAN, COLOR_RESET);
            fflush(stdout);
        }
